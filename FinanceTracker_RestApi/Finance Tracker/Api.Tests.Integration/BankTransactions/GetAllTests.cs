...
public class GetAllTests : BaseIntegrationTest, IAsyncLifetime
{
    ...
    public async Task GetAllTransactions_Success()
    {
        ...
        // Act
        var response = await Client.GetAsync("bankTransactions/getAll/");

        // Assert

        response.EnsureSuccessStatusCode();
        
        var transactionInDb = await Context.BankTransactions.ToListAsync();
        
        Assert.Contains(transactionInDb, t => t.Id == _bankTransaction1.Id); 
        Assert.Contains(transactionInDb, t => t.Id == _bankTransaction2.Id); 
    }
